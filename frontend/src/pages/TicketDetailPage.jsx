...
const P_CONFIG = {
  CRITICAL: { color: "#e74c3c", bg: "rgba(231,76,60,0.13)", label: "Critique" },
  HIGH: { color: "#e67e22", bg: "rgba(230,126,34,0.12)", label: "Élevée" },
  MEDIUM: { color: "#f39c12", bg: "rgba(243,156,18,0.12)", label: "Moyenne" },
  LOW: { color: "#27ae60", bg: "rgba(39,174,96,0.12)", label: "Faible" },
};
...
export default function TicketDetailPage() {
  ...
  const canChangeStatus = (() => {
    ...
    if (user?.specialty && ticket?.category?.name) {
      const normalizeObj = (str) => str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
      return normalizeObj(ticket.category.name).includes(normalizeObj(user.specialty));
    }
    return false;
  })();
  const pc = P_CONFIG[ticket.priority] || P_CONFIG.LOW;
              ...
              {(ticket.comments || []).map((c) => (
                <div key={c.id} className="comment">
                  <div className="comment-header">
                    <span className="comment-author">{c.authorName || `${c.author?.firstName || ""} ${c.author?.lastName || ""}`}</span>
...
const DEMO_TICKET = {
  ...
  priority: "CRITIQUE", status: "OUVERT", category: { name: "Non défini" },
