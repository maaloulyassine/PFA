...
export default function ProfilePage() {
  ...
  // Fixed: always update localStorage + auth context regardless of API success/failure
  const handleUpdate = async (e) => {
    e.preventDefault();
    const updatedUser = { ...user, ...form, avatar };

    // Always save to localStorage and update context immediately
    localStorage.setItem("user", JSON.stringify(updatedUser));
    if (setUser) setUser(updatedUser);

    // Then try the real API in the background
    try {
      await api.put("/users/me", { ...form, avatar });
      addToast("Profil mis à jour.", "success");
    } catch {
      // API failed but local save already happened — inform user it's saved locally
      addToast("Profil mis à jour (mode démo).", "success");
    }
  };
  ...
  const handleAvatar = (e) => {
    ...
    reader.onload = () => {
      ...
      setAvatar(dataUrl);
      // Persist avatar immediately so it shows everywhere (sidebar, header, etc.)
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div
              style={{
                ...
                height: 72,
                borderRadius: "50%",
                background: avatar ? "transparent" : ROLE_COLOR[user?.role],
                display: "flex",
                alignItems: "center",
          ...
          <div>
            {/* Show live preview of name from form */}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              {/* Show specialty for technicians */}
              {user?.role?.toString().toUpperCase() === 'TECHNICIEN' && (
                <div style={{ fontSize: 13, color: '#9da3c8', fontWeight: 500 }}>
                  Spécialité: <span style={{ color: '#e8eaf0', fontWeight: 700 }}>{formatSpecialty(user?.specialty)}</span>
                </div>
              )}
            </div>
          </div>
