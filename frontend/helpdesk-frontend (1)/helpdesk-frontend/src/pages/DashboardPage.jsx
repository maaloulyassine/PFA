...
// ── Priority config ────────────────────────────────────────────────────────
const P = {
  CRITIQUE: { color: "#f87171", bg: "rgba(231,76,60,0.13)", border: "rgba(231,76,60,0.35)", label: "Critique", sla: "2 heures" },
  ELEVEE:   { color: "#fb923c", bg: "rgba(230,126,34,0.12)", border: "rgba(230,126,34,0.3)",  label: "Élevée",   sla: "8 heures" },
  MOYENNE:  { color: "#fbbf24", bg: "rgba(243,156,18,0.12)", border: "rgba(243,156,18,0.3)",  label: "Moyenne",  sla: "48 heures" },
  FAIBLE:   { color: "#4ade80", bg: "rgba(39,174,96,0.12)",  border: "rgba(39,174,96,0.3)",   label: "Faible",   sla: "5 jours" },
};
      ...
      <section className="section">
        ...
        <div className="ticket-list">
          {tickets.slice(0, 8).map((t) => {
            const pc = P[t.priority] || P.FAIBLE;
...
const SLA_DESC = {
  CRITIQUE: "Panne majeure, intervention immédiate requise",
  ELEVEE:   "Incident bloquant la productivité",
  MOYENNE:  "Gênant mais contournement possible",
  FAIBLE:   "Demande mineure sans impact direct",
};

const DEMO_TICKETS = [
  { id: 1, title: "Panne réseau salle informatique B12 — 30 postes HS", priority: "CRITIQUE", status: "OUVERT",   category: { name: "Réseau" },   impactedUsers: 30, createdAt: new Date().toISOString() },
  { id: 2, title: "Virus détecté poste #41 — fichiers chiffrés",         priority: "CRITIQUE", status: "EN_COURS", category: { name: "Sécurité" }, impactedUsers: 5,  createdAt: new Date().toISOString() },
  { id: 3, title: "Imprimante bloc admin HS — dossiers urgents bloqués", priority: "ELEVEE",   status: "OUVERT",   category: { name: "Matériel" }, impactedUsers: 8,  createdAt: new Date().toISOString() },
  { id: 4, title: "Mise à jour MATLAB — salle TP réseau",                priority: "MOYENNE",  status: "EN_COURS", category: { name: "Logiciel" }, impactedUsers: 3,  createdAt: new Date().toISOString() },
  { id: 5, title: "Compte VPN étudiant bloqué",                          priority: "FAIBLE",   status: "RESOLU",   category: { name: "Réseau" },   impactedUsers: 1,  createdAt: new Date().toISOString() },
];
